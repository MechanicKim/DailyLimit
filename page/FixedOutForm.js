...
export default class FixedOutForm extends Component {
  ...
  render() {
    ...
    let lastTotal = parseInt(total || 0);
    outRecords.forEach(record => { lastTotal -= parseInt(record.out || 0); });

    return (
      <Page>
        <StatusBar barStyle="default" />
        <BackButton />
        <FixedOutRecords records={outRecords} update={this.openUPdate} remove={this.removeOut} />
        <FixedOutBoard total={total} onChangeTotal={this.onChangeTotal} lastTotal={lastTotal} />
        <FixedOutFooter back={this.props.history.goBack}
                        add={this.openPopup} save={this.saveOut} />
        {popupOn && <Popup close={this.closePopup} add={this.addOut} record={record} />}
      </Page>
    );
  }
  ...
  saveOut = () => {
    ...
    outRecords.forEach(record => {
      finalTotal -= parseInt(record.out || 0);
    });
    if (finalTotal < 0) {
    ...
    realm.write(() => {
      ...
      realm.create('FixedIn', { in: parseInt(total), limit });
      outRecords.forEach((record, index) => {
        if (!record.desc || !record.out) return;
        realm.create('FixedOutRecord', {
          id: index,
          desc: record.desc,
          out: parseInt(record.out)
        });
      });
      ...
      for (let d = 0; d <= recordDays; d++) {
        const id = parseInt(moment(start, 'YYYYMMDD').add(d, 'd').format('YYYYMMDD'));
