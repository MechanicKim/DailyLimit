...
import { FixedInSchema, FixedOutSchema, TodaySchema, OutRecordSchema, SchemaVersion } from '../component/Schema';
...
export default class Main extends Component {
  ...
  componentDidMount() {
    ...
    Realm.open({
      ...
      realm.write(() => {
        ...
        /* 시작에서 끝까지 지출 내역 불러오기 */
        const query = `id >= ${start.format('YYYYMMDD')} AND id <= ${end.format('YYYYMMDD')}`;
        const dayRecords = realm.objects('Today').filtered(query).sorted('id', true);

        /* 일별 잔액 갱신 */
        const days = end.diff(start, 'days');
        let balance = 0;
        for (let d = 0; d <= days; d++) {
          const id = parseInt(moment(start).add(d, 'd').format('YYYYMMDD'));
  ...
  render() {
    ...
    return (
      <Page>
        ...
        <BackButton />
        <MainTop today={dayRecords[0] || {}} fixedLimit={dailyLimit} total={total} goToPage={this.goToPage} />
        <MainRecords fixedLimit={dailyLimit} dayRecords={dayRecords} goToPage={this.goToPage} />
  ...
  getRecords = (add) => {
    ...
    const newRecords = realm.objects('Today').filtered(query).sorted('id', true);
